...
public class DataReader {
    ...
    public DataReader(JDialog dialog, SearchPanel searchPanel) {
        this.dialog = dialog;
    ...
    public boolean isValid(int surname, int firstName, int patronymic) {
        ...
        if (!isOne(surname) && !isOne(firstName) && !isOne(patronymic)) {
            isValid = false;
            JOptionPane.showMessageDialog(dialog, "Select the findStudent parameter", "Parameter error", JOptionPane.ERROR_MESSAGE);
    ...
    private void showMassage(JTextField textField, String mistakeName) {
        JOptionPane.showMessageDialog(dialog, mistakeName + " was incorrectly entered!");
        textField.setText("");
